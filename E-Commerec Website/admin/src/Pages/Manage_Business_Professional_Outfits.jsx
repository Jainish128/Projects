...
function Manage_Business_Professional_Outfits() {
  ...
  const fetch = async () => {
    const res = await axios.get('http://localhost:3000/businessprofessional');
  ...
  const onDelete = async(id) => {
    const res=await axios.delete(`http://localhost:3000/businessprofessional/${id}`)
    // console.log(res);
    if (res.status == "200") 
    {
      fetch();  
                              ...
                              data.map((value, index, entarr) => {
                                return (
                                  <tr>
                                    ...
                                    <td> <img src={value.img_url} width={"200px"} height={"250px"} /> </td>
                                    <td> </td>
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                                      ...
                                      <button className="btn btn-primary" onClick={()=> {redirect('/Edit_Business_Professional_Outfits/' + value.id )}}> 
                                        Edit 
                                      </button> 
...
export default Manage_Business_Professional_Outfits