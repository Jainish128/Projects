...
function Manage_Categories() {
  ...
  // ---------------| Fetch Categories Data from JSON-Server and Show in Manage Category Table -> START |---------------
  useEffect(() => {
    fetch_categories();
  });
  ...
  const fetch_categories = async() => {
    ...
    setCateData(categories.data);
  }
  // ---------------| Fetch Categories Data from JSON-Server and Show in Manage Category Table -> END |---------------


  // ---------------| Delete Category from Manage Categories webpage -> START |---------------
  const deleteCategory = async(id) => {
    const resp = await axios.delete(`http://localhost:3000/categories/${id}`);
    if (resp.status == 200) 
    {
  ...
  // ---------------| Delete Category from Manage Categories webpage -> END |---------------
        ...
        <div id="content-wrapper" className="d-flex flex-column">
          ...
          <div id="content">
            ...
            <div className="container-fluid">
              {/* Page Heading */}
              <h1 className="h3 mb-2 text-gray-800 text-center border border-primary py-2 rounded"> Manage Categories </h1>
              <p className="mb-4">
                You Can Read, Edit & Delete Categories in this Webpage.
              </p>
              {/* DataTales Example */}
...

                        {
                          cateData.map((value, index) => {
                            return (
                              <tr key={index}>
                                <td> {value.id} </td>
                                <td style={{ width: '150px' }}> {value.cate_name} </td>
                                <td style={{ width: '170px' }}> 
                                  <img src={value.cate_img} alt="Category-Image" width={150} height={150} /> 
                                </td>
                                <td> {value.cate_desc} </td>
                                <td style={width125px}> {value.created_at} </td>
                                <td style={width125px}> {value.updated_at} </td>
                                <td style={width125px}>  
                                    <button className="btn btn-sm btn-primary btn-icon-split m-1" title="Edit">
                                      <span className="icon">
                                    ...
                                    <button className="btn btn-sm btn-danger btn-icon-split me-3 ms-2" onClick={()=> deleteCategory(value.id)} title="Edit">
                                      <span className="icon">
