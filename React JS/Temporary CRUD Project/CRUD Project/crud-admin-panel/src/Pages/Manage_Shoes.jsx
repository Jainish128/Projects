...
function Manage_Shoes() {
  ...
  const fetch_shoes = async() => {
    const resp = await axios.get(`http://localhost:3000/shoes`);
    setShoes(resp.data);
  };
  // ---------------| Fetch Shirts Data from JSON-Server and Show in Manage Shirts Table -> END |---------------
   ...
   const delete_Shoes = async(id) => {
    const resp = await axios.delete(`http://localhost:3000/shoes/${id}`);
    if (resp.status == 200) 
    {
      toast.success('Shoes product was deleted successfully.');
      fetch_shoes();
    }
   };
                              ...
                              <tr key={index} align='center'>
                                ...
                                <td>
                                  <div style={{ width: "125px" }}>
                                    ...
                                    <button className="btn btn-sm btn-danger btn-icon-split mb-1" onClick={()=> delete_Shoes(value.id)} title="Edit">
                                      <span className="icon">
